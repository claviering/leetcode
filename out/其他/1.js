 ...
 * @param m
 */
function walk_leftTop_to_rightDown(n, m) {
    // base case
    if (n <= 0 || m <= 0)
        return 0;
    if (n === 1 || m === 1)
        return 1;
    let mar = new Array(n).fill(new Array(m).fill(1));
    for (let i = 1; i < n; i++) {
        for (let j = 1; j < m; j++) {
 ...
 * @param m
 */
function walk_leftTop_to_rightDown_comp(n, m) {
    // base case
    if (n <= 0 || m <= 0)
        return 0;
    if (n === 1 || m === 1)
        return 1;
    let m_list = new Array(m).fill(1);
    for (let i = 1; i < n; i++) {
        let left = 1;
        for (let j = 1; j < m; j++) {
